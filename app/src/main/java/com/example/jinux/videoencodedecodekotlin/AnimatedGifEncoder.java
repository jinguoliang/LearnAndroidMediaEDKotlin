...
import com.example.jinux.videoencodedecodekotlin.utils.CancelledException;
import com.example.jinux.videoencodedecodekotlin.utils.LZWEncoder;
import com.example.jinux.videoencodedecodekotlin.utils.NeuQuant;
...
public class AnimatedGifEncoder {
    ...
    private String mComment; // text which is not display
    ...
    public boolean addFrame(Bitmap im) throws CancelledException {
        return addFrame(im, delay);
    }
     ...
     */
    public boolean addFrame(Bitmap im, long frameDelay) throws CancelledException {
        if ((im == null) || !started) {
            return false;
        }
        boolean ok = true;
        try {
            ...
            if (isCancelled) {
                throw new CancelledException("cancel before getImagePixels");
            }

            getImagePixels(); // convert to correct format if necessary

            if (isCancelled) {
                throw new CancelledException("cancel before analyzePixels");
            }

            analyzePixels(); // build color table & map pixels

            if (isCancelled) {
                throw new CancelledException("cancel before writeGraphicCtrlExt");
            }
            ...
            if (isCancelled) {
                throw new CancelledException("cancel before writePixels");
            }
    ...
    private NeuQuant mCurrentNQ;

    /**
     * Analyzes image colors and creates color map.
     */
    private void analyzePixels() throws CancelledException {
        int len = pixels.length;
        int nPix = len / 3;
        indexedPixels = new byte[nPix];
        NeuQuant nq = NeuQuant.getReusableNet(pixels, len, sample);
        mCurrentNQ = nq;
        // initialize quantizer
        byte[] tmpColorTable = nq.process();
        colorTab = Arrays.copyOf(tmpColorTable, tmpColorTable.length); // create reduced palette
        mCurrentNQ = null;
     ...
     * @param frameDelay
     */
    private void writeGraphicCtrlExt(long frameDelay) throws IOException {
        ...
        writeShort(Math.round(frameDelay / 10.0f)); // delay x 1/100 sec
    ...
    private void writeComment() throws IOException {
        ...
        byte[] bytes = mComment.getBytes();
        int count = bytes.length;
        int blockCount = count / 0xff;
        int lastBlockCount = count % 0xff;

        for (int i = 0; i < blockCount; i++) {
            out.write(0xff); // sub block size
            writeBytes(bytes, i * 0xff, (i + 1) * 0xff); // data
        }
        out.write(lastBlockCount); // sub block size
        writeBytes(bytes, blockCount * 0xff, bytes.length);
        out.write(0); // block terminator
    }

    private void writeBytes(byte[] bytes, int start, int end) throws IOException {
